...
import {
    ...
    Search, User, MoreHorizontal, Phone, CalendarClock, X, Bell, ShieldCheck, FileText, Receipt, Eye, Plus, Check, MessageCircle, Mic, Sparkles
} from "lucide-react";
...
export function AppointmentsHub() {
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="hidden md:flex flex-col items-end">
                                    ...
                                    <span className="text-xs font-black text-emerald-600">₹{MOCK_REVENUE.toLocaleString()}</span>
                                </div>

                                {/* Smart Standby Widget (New) */}
                                <div className="hidden md:flex items-center gap-2 bg-indigo-50/50 px-3 py-1.5 rounded-xl border border-indigo-100/50">
                                    <div className="flex flex-col items-end">
                                        <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400">Smart Waitlist</span>
                                        <span className="text-xs font-black text-indigo-700 flex items-center gap-1">
                                            3 Patients <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
                                        </span>
                                    </div>
                                </div>
    ...
    const procedure = PROCEDURE_TYPES.find(p => p.id === appt.type);
            ...
            const [activeTab, setActiveTab] = useState<'overview' | 'medical' | 'billing'>('overview');
            ...
            // Mock "Smart" Logic
            // In a real app, calculate if previous appt is overflowing.
            const isDelayed = isUpcoming && parseInt(appt.slot.split(':')[1]) % 2 !== 0; // Random deterministic mock (odd minutes = delay)
    ...
    const getSmartAction = () => {
        if (isUpcoming) return {label: "Send Reminder", icon: Bell, action: () => toast.success("Reminder sent via WhatsApp") };
            if (isArrived) return {label: "Verify Insurance", icon: ShieldCheck, action: () => toast.success("Insurance verified") };
            if (isOngoing) return {label: "Add Clinical Note", icon: FileText, action: () => toast.success("Note draft created") };
            if (isCompleted) return {label: "Generate Invoice", icon: Receipt, action: () => toast.success("Invoice generated") };
                ...
                <div className="relative z-10 pointer-events-none">
                    ...
                    <div className={cn(
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {isDelayed && (
                                <Badge className="bg-red-50 text-red-600 border border-red-100 shadow-none uppercase text-[9px] tracking-wider flex items-center gap-1">
                                    <Clock className="w-3 h-3" /> +{delayMinutes}m Delay
                                </Badge>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-1 pointer-events-auto">
                            ...
                            <Button
                                ...
                                size="sm"
                                className={cn(
                                    "h-8 rounded-full transition-all gap-2",
                                    isRecording ? "bg-red-50 text-red-600 w-auto px-3" : "w-8 px-0 text-slate-400 hover:text-slate-600"
                                )}
                                onClick={(e) => { e.stopPropagation(); setIsRecording(!isRecording); if (!isRecording) toast.info("AI Scribe Listening..."); }}
                            >
                                {isRecording ? <><div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /><span className="text-[10px] font-bold uppercase">Rec</span></> : <Mic className="w-3.5 h-3.5" />}
                            </Button>
                            <Dialog>
                    ...
                    <div className="p-6 flex gap-5 relative">
                        ...
                        <div className="flex-1 min-w-0 pr-10">
                            ...
                            {/* Quick Contact & Smart Action (Pointer events auto to allow clicking) */}
                            <div className="flex items-center gap-2 pointer-events-auto">
                                <Button
                                    variant="outline"
                                    size="sm"
                                    className="h-8 text-[10px] font-bold uppercase tracking-wider gap-2 rounded-lg border-slate-200 text-slate-500 hover:text-indigo-600 hover:bg-slate-50"
                                    onClick={(e) => { e.stopPropagation(); smartAction.action(); }}
                                >
                                    <smartAction.icon className="w-3.5 h-3.5" /> {smartAction.label}
                                </Button>
                                <a
                                    href={`https://wa.me/${patient?.phone?.replace(/\D/g, '')}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all border border-slate-100 active:scale-95"
                            ...
                            >
                                <div className="pointer-events-auto">
                                    ...
                                    <div className="flex items-center px-6 border-b border-slate-50">
                                        {['overview', 'medical', 'billing'].map((tab) => (
                                            <button
                                                key={tab}
                                            ...
                                            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                                                        <h4 className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Last Visit</h4>
                                                        <p className="text-xs font-semibold text-slate-700">{lastVisit}</p>
                                                    </div>
                                                    <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                                                        <h4 className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Next Follow-up</h4>
                                                        <p className="text-xs font-semibold text-slate-700">Not Scheduled</p>
                                                    </div>
                                                </div>
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="relative">
                                                        <textarea
                                                            className="w-full text-xs bg-white border-slate-100 rounded-xl p-3 focus:ring-1 focus:ring-indigo-200 resize-none h-20 placeholder:text-slate-300 text-slate-700 shadow-sm"
                                                            placeholder="Type a note..."
                                                        />
                                                        <Button size="icon" className="absolute bottom-2 right-2 h-6 w-6 rounded-lg bg-indigo-600 hover:bg-indigo-700" onClick={(e) => e.stopPropagation()}>
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        {medicalAlerts.map((alert: string, i: number) => (
                                                            <div key={i} className="bg-red-50 text-red-600 px-3 py-2 rounded-lg text-xs font-bold border border-red-100 flex items-center gap-2">
                                                                <AlertCircle className="w-3.5 h-3.5" /> {alert}
                                                            </div>
                                                        ))}
                                                        <button
                                                            className="px-3 py-2 rounded-lg text-xs font-bold border border-dashed border-slate-300 text-slate-400 hover:border-slate-400 hover:text-slate-600 flex items-center gap-2"
                                                            onClick={(e) => e.stopPropagation()}
                                                        >
                                            ...
                                            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                                                <div className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
                                                    ...
                                                    <div className="px-4 py-3 flex justify-between items-center border-b border-slate-50">
                                                        <span className="text-xs font-medium text-slate-700">Root Canal Treatment</span>
                                                        <span className="text-xs font-bold text-slate-900">₹4,500</span>
                                                    </div>
                                                    <div className="px-4 py-3 flex justify-between items-center bg-indigo-50/30">
                                                        <span className="text-xs font-bold text-slate-900">Total Due</span>
                                                        <span className="text-xs font-black text-indigo-600">₹6,000</span>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <Button className="flex-1 bg-slate-900 text-white h-9 rounded-lg text-xs font-bold" onClick={(e) => e.stopPropagation()}>
                                                        Generate Invoice
                                                    </Button>
                                                    <Button variant="outline" className="flex-1 border-slate-200 text-slate-600 h-9 rounded-lg text-xs font-bold" onClick={(e) => e.stopPropagation()}>
                                                        View History
                                                    </Button>
                                                </div>
                                            </motion.div>
