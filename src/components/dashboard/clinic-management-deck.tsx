            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-5 auto-rows-min pb-20">
                {MANAGEMENT_FEATURES.map((feature, i) => {
                    const CardContent = (
                        ...
                        >
                            {/* Professional Pure White Glow */}
                            <div className="absolute -inset-[1px] bg-white/[0.08] rounded-[2.2rem] opacity-0 group-hover:opacity-100 blur-md transition-all duration-700 -z-10" />
                            <div className="absolute -inset-4 bg-white/[0.03] rounded-[3rem] opacity-0 group-hover:opacity-100 blur-2xl transition-all duration-1000 -z-10" />

                            {/* High-Fidelity Obsidian Glass Card */}
                            <div className="bg-slate-950/60 backdrop-blur-3xl rounded-[2.2rem] overflow-hidden relative p-8 h-full flex flex-col min-h-[185px] border border-white/5 shadow-[0_30px_60px_-12px_rgba(0,0,0,0.5)] transition-all duration-700 group-hover:border-white/20 group-hover:bg-black/80">
                                ...
                                <div className="flex justify-between items-start relative z-10 mb-8">
                                    <div className="w-11 h-11 rounded-2xl bg-white/5 flex items-center justify-center text-white/50 border border-white/10 group-hover:bg-white group-hover:text-black group-hover:scale-105 group-hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all duration-500">
                                    ...
                                    {feature.locked ? (
                                        ...
                                        <div className="bg-white/5 border border-white/10 text-[8px] uppercase font-bold tracking-[0.25em] px-3 py-1 rounded-full text-white/40 shadow-[0_0_15px_rgba(255,255,255,0.05)]">
                                            {feature.badge}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="mt-auto relative z-10">
                                    <h3 className="text-xl font-semibold tracking-tight text-white mb-1.5 leading-none transition-transform duration-500 group-hover:translate-x-1">
                                        {feature.title}
                                    </h3>
                                    <p className="text-[11px] text-white/30 font-medium leading-tight group-hover:text-white/50 transition-all duration-500 group-hover:translate-x-1">
                                        {feature.subtitle}
                                    </p>
                                </div>

                                {/* Refined White Interaction Pulse (Bottom Line) */}
                                {!feature.locked && (
                                    <div className="absolute bottom-5 left-8 right-8 h-[1.5px] bg-white/5 rounded-full overflow-hidden">
                                        <motion.div
                                            initial={{ x: '-100%' }}
                                            whileHover={{ x: '0%' }}
